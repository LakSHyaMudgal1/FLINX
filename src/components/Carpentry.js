  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center">
      <div className="bg-[#1e1e1e] p-8 rounded-lg shadow-lg w-full max-w-xl relative">
        ...
        <h2 className="text-2xl font-bold text-white mb-4">Register as a Carpenter</h2>
...
const CarpentryService = () => {
  ...
  useEffect(() => {
    const fetchTechnicians = async () => {
      const techQuerySnapshot = await getDocs(collection(db, 'carpenters'));
      const techList = techQuerySnapshot.docs.map((doc) => ({ id: doc.id, ...doc.data() }));
      setTechnicians(techList);
    };

    const fetchCartItems = async () => {
      const cartQuerySnapshot = await getDocs(collection(db, 'carpentryCart'));
  ...
  const addTechnician = async (technician) => {
    try {
      await addDoc(collection(db, 'carpenters'), technician);
  ...
  const addToCart = async (technician) => {
    try {
      ...
      const docRef = await addDoc(collection(db, 'carpentryCart'), cartItem);
  ...
  const removeFromCart = async (id) => {
    try {
      await deleteDoc(doc(db, 'carpentryCart', id));
  ...
  const services = [
    { name: 'Furniture Assembly', minPrice: 30, maxPrice: 100 },
    { name: 'Custom Cabinets', minPrice: 100, maxPrice: 500 },
    { name: 'Wood Repairs', minPrice: 40, maxPrice: 150 },
    { name: 'Deck Construction', minPrice: 200, maxPrice: 1000 },
    { name: 'Door Installation', minPrice: 50, maxPrice: 300 },
  ];
  ...
  return (
    <div className="bg-[#121212] min-h-screen flex flex-col items-center text-white">
      <header className="w-full py-8 bg-gray-900 text-center">
        <h1 className="text-4xl font-bold">Carpentry Services</h1>
        <button
      ...
      <section className="w-full max-w-4xl px-4 py-16">
        <h2 className="text-3xl font-semibold mb-6">Expert Carpentry Services</h2>
        ...
        {showTechnicianList && (
          <div className="absolute inset-0 bg-black bg-opacity-60 flex justify-center items-center">
            <div className="bg-[#1e1e1e] p-8 rounded-lg shadow-lg w-full max-w-xl">
              <h3 className="text-xl font-semibold mb-4">Select a Carpenter for {selectedService}</h3>
              {filteredTechnicians.length === 0 ? (
                <p className="text-gray-400">No carpenters available for this service.</p>
              ) : (
