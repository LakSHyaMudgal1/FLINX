  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center">
      <div className="bg-[#1e1e1e] p-8 rounded-lg shadow-lg w-full max-w-xl relative">
        ...
        <h2 className="text-2xl font-bold text-white mb-4">Register as a Plumber</h2>
...
<Link to="/combined-cart">
  ...
  useEffect(() => {
    const fetchTechnicians = async () => {
      const techQuerySnapshot = await getDocs(collection(db, 'plumbers'));
      const techList = techQuerySnapshot.docs.map((doc) => ({ id: doc.id, ...doc.data() }));
      setTechnicians(techList);
    };

    const fetchCartItems = async () => {
      const cartQuerySnapshot = await getDocs(collection(db, 'plumbingCart'));
  ...
  const addTechnician = async (technician) => {
    try {
      await addDoc(collection(db, 'plumbers'), technician);
  ...
  const addToCart = async (technician) => {
    try {
      ...
      const docRef = await addDoc(collection(db, 'plumbingCart'), cartItem);
  ...
  const removeFromCart = async (id) => {
    try {
      await deleteDoc(doc(db, 'plumbingCart', id));
  ...
  const services = [
    { name: 'Pipe Installation', minPrice: 100, maxPrice: 400 },
    { name: 'Drain Cleaning', minPrice: 50, maxPrice: 200 },
    { name: 'Leak Repairs', minPrice: 80, maxPrice: 300 },
    { name: 'Water Heater Installation', minPrice: 200, maxPrice: 600 },
    { name: 'Toilet Repair', minPrice: 100, maxPrice: 300 },
  ];
  ...
  return (
    <div className="bg-[#121212] min-h-screen flex flex-col items-center text-white">
      ...
      <section className="w-full max-w-4xl px-4 py-16">
        <h2 className="text-3xl font-semibold mb-6">Expert Plumbing Services</h2>
        ...
        {showTechnicianList && (
          <div className="absolute inset-0 bg-black bg-opacity-60 flex justify-center items-center">
            <div className="bg-[#1e1e1e] p-8 rounded-lg shadow-lg w-full max-w-xl">
              <h3 className="text-xl font-semibold mb-4">Select a Plumber for {selectedService}</h3>
              {filteredTechnicians.length === 0 ? (
                <p className="text-gray-400">No plumbers available for this service.</p>
              ) : (
