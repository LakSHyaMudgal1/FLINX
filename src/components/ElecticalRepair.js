  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center">
      <div className="bg-[#1e1e1e] p-8 rounded-lg shadow-lg w-full max-w-xl relative">
        ...
        <h2 className="text-2xl font-bold text-white mb-4">Register as a Technician</h2>
...
<Link to="/combined-cart">
  ...
  useEffect(() => {
    const fetchTechnicians = async () => {
      const techQuerySnapshot = await getDocs(collection(db, 'technicians'));
      const techList = techQuerySnapshot.docs.map((doc) => ({ id: doc.id, ...doc.data() }));
      setTechnicians(techList);
    };

    const fetchCartItems = async () => {
      const cartQuerySnapshot = await getDocs(collection(db, 'addedtocart'));
  ...
  const addTechnician = async (technician) => {
    try {
      await addDoc(collection(db, 'technicians'), technician);
  ...
  const addToCart = async (technician) => {
    try {
      ...
      const docRef = await addDoc(collection(db, 'addedtocart'), cartItem);
  ...
  const removeFromCart = async (id) => {
    try {
      await deleteDoc(doc(db, 'addedtocart', id));
  ...
  const services = [
    { name: 'Wiring Installation', minPrice: 50, maxPrice: 150 },
    { name: 'Electrical Repairs', minPrice: 40, maxPrice: 120 },
    { name: 'Lighting Installation', minPrice: 60, maxPrice: 200 },
    { name: 'Circuit Breaker Service', minPrice: 80, maxPrice: 250 },
    { name: 'Home Automation', minPrice: 100, maxPrice: 300 },
  ];

  // Filter technicians based on the selected service
  ...
  return (
    <div className="bg-[#121212] min-h-screen flex flex-col items-center text-white">
      <header className="w-full py-8 bg-gray-900 text-center">
        <h1 className="text-4xl font-bold">Electrical Services</h1>
        <button
      ...
      <section className="w-full max-w-4xl px-4 py-16">
        <h2 className="text-3xl font-semibold mb-6">Professional Electrical Services</h2>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {services.map((service, index) => (
            ...
            >
              <div className="mb-4">
              {/* <div className="bg-gray-700 h-32 w-full rounded-lg mb-4"></div> */}
              ...
              <button
                onClick={() => {
                  setSelectedService(service.name);  // Update selected service before showing technicians
                  setShowTechnicianList(true); // Show the technician list modal
                }}
        ...
        {showTechnicianList && (
          <div className="absolute inset-0 bg-black bg-opacity-60 flex justify-center items-center">
            <div className="bg-[#1e1e1e] p-8 rounded-lg shadow-lg w-full max-w-xl">
              <h3 className="text-xl font-semibold mb-4">Select a Technician for {selectedService}</h3>
              {filteredTechnicians.length === 0 ? (
                <p className="text-gray-400">No technicians available for this service.</p>
              ) : (
                    ...
                    >
                      ...
                      <button
                        onClick={() => {
                          ...
                          setShowTechnicianList(false); // Close technician list after adding
                        }}
