  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center">
      <div className="bg-[#1e1e1e] p-8 rounded-lg shadow-lg w-full max-w-xl relative">
        ...
        <h2 className="text-2xl font-bold text-white mb-4">Register as a Pest Control Provider</h2>
        ...
        <form className="space-y-4" onSubmit={handleSubmit}>
          ...
          <div>
            ...
            <input
              ...
              className="w-full p-3 rounded-lg bg-[#3c3c3c] text-gray-200 border border-gray-600 focus:outline-none"
              placeholder="Enter your price per session"
              required
            />
          </div>
            ...
            {cartItems.map((item, index) => (
              ...
              >
                <div>
                  ...
                  {item.contact} - Price: ${item.price} per session
                </div>
                <button
...
<Link to="/combined-cart">
  ...
  useEffect(() => {
    const fetchProviders = async () => {
      const providerQuerySnapshot = await getDocs(collection(db, 'pestControlProviders'));
      const providerList = providerQuerySnapshot.docs.map((doc) => ({ id: doc.id, ...doc.data() }));
      setProviders(providerList);
    };

    const fetchCartItems = async () => {
      const cartQuerySnapshot = await getDocs(collection(db, 'pestControlCart'));
  ...
  const addProvider = async (provider) => {
    try {
      await addDoc(collection(db, 'pestControlProviders'), provider);
  ...
  const addToCart = async (provider) => {
    try {
      ...
      const docRef = await addDoc(collection(db, 'pestControlCart'), cartItem);
  ...
  const removeFromCart = async (id) => {
    try {
      await deleteDoc(doc(db, 'pestControlCart', id));
  ...
  const services = [
    { name: 'Ant Control', minPrice: 50, maxPrice: 150 },
    { name: 'Cockroach Control', minPrice: 60, maxPrice: 200 },
    { name: 'Termite Treatment', minPrice: 100, maxPrice: 300 },
    { name: 'Bed Bug Treatment', minPrice: 80, maxPrice: 250 },
    { name: 'Rodent Control', minPrice: 70, maxPrice: 200 },
  ];
  ...
  return (
    <div className="bg-[#121212] min-h-screen flex flex-col items-center text-white">
      <header className="w-full py-8 bg-gray-900 text-center">
        <h1 className="text-4xl font-bold">Pest Control Services</h1>
        <button
      ...
      <section className="w-full max-w-4xl px-4 py-16">
        <h2 className="text-3xl font-semibold mb-6">Protect Your Home with Professional Pest Control</h2>
              ...
              {filteredProviders.length === 0 ? (
                ...
                <div className="space-y-4">
                  {filteredProviders.map((provider) => (
                    ...
                    >
                      <span>{provider.name} - ${provider.price} per session</span>
                      <button
                        onClick={() => {
                          addToCart(provider);
