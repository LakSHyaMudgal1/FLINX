  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center">
      <div className="bg-[#1e1e1e] p-8 rounded-lg shadow-lg w-full max-w-xl relative">
        ...
        <h2 className="text-2xl font-bold text-white mb-4">Register as a Painter</h2>
...
<Link to="/combined-cart">
  ...
  useEffect(() => {
    const fetchTechnicians = async () => {
      const techQuerySnapshot = await getDocs(collection(db, 'painters'));
      const techList = techQuerySnapshot.docs.map((doc) => ({ id: doc.id, ...doc.data() }));
      setTechnicians(techList);
    };

    const fetchCartItems = async () => {
      const cartQuerySnapshot = await getDocs(collection(db, 'paintingCart'));
  ...
  const addTechnician = async (technician) => {
    try {
      await addDoc(collection(db, 'painters'), technician);
  ...
  const addToCart = async (technician) => {
    try {
      ...
      const docRef = await addDoc(collection(db, 'paintingCart'), cartItem);
  ...
  const removeFromCart = async (id) => {
    try {
      await deleteDoc(doc(db, 'paintingCart', id));
  ...
  const services = [
    { name: 'Interior Painting', minPrice: 150, maxPrice: 500 },
    { name: 'Exterior Painting', minPrice: 200, maxPrice: 1000 },
    { name: 'Fence Painting', minPrice: 50, maxPrice: 300 },
    { name: 'Wall Texturing', minPrice: 100, maxPrice: 700 },
    { name: 'Decorative Painting', minPrice: 250, maxPrice: 1000 },
  ];
  ...
  return (
    <div className="bg-[#121212] min-h-screen flex flex-col items-center text-white">
      ...
      <section className="w-full max-w-4xl px-4 py-16">
        <h2 className="text-3xl font-semibold mb-6">Professional Painting Services</h2>
        ...
        {showTechnicianList && (
          <div className="absolute inset-0 bg-black bg-opacity-60 flex justify-center items-center">
            <div className="bg-[#1e1e1e] p-8 rounded-lg shadow-lg w-full max-w-xl">
              <h3 className="text-xl font-semibold mb-4">Select a Painter for {selectedService}</h3>
              {filteredTechnicians.length === 0 ? (
                <p className="text-gray-400">No painters available for this service.</p>
              ) : (
